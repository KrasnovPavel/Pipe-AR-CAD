...
namespace HoloCAD.UI
{
    ...
    public class StartTubeFragmentControlPanel : TubeFragmentControlPanel
    {
        ...
        [Tooltip("Кнопка увеличения диаметра трубы.")] [CanBeNull]
        public Button3D IncreaseDiameterButton;

        /// <summary> Кнопка уменьшения диаметра трубы. </summary>
        [Tooltip("Кнопка уменьшения диаметра трубы.")] [CanBeNull]
        public Button3D DecreaseDiameterButton;

        /// <summary> Кнопка перехода в режим размещения трубы. </summary>
        [Tooltip("Кнопка перехода в режим размещения трубы.")] [CanBeNull]
        public Button3D StartPlacingButton;

        /// <summary> Кнопка сохранения сцены. </summary>
        [Tooltip("Кнопка добавления объекта отображения расстояния между трубами.")] [CanBeNull]
        public Button3D SaveSceneButton;

        /// <summary> Кнопка создания новой трубы. </summary>
        [Tooltip("Кнопка создания новой трубы.")] [CanBeNull]
        ...
        protected override void CalculateBarPosition()
        {
            ...
            barPosition.x = (_fragment.Diameter / 2 + 0.3f) * (cameraPosition.ProjectOn(transform.right) > 0 ? 1 : -1);
            barPosition.y = (_fragment.Diameter / 2 + 0.2f) * (cameraPosition.ProjectOn(transform.up) > 0 ? 1 : -1);
        ...
        private void CalculateLine()
        {
            ...
            linePositions[2] = barPosition + Vector3.right 
                                                * (cameraPosition.ProjectOn(transform.right) > 0 ? 0.5f : -0.5f);
