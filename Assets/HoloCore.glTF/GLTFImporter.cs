...
namespace HoloCore.GLTF
{
    ...
    {        
        ...
        /// <summary> Импортирует объект из файла </summary>
        /// <param name="target"> GameObject к которому будет добавлен меш</param>
        /// <returns> Метки найденные в файле </returns>
        public static async Task<List<Mark>> ImportGLTFFile(GameObject target)
        {
            (string json, string _) = await UnityFileManager.PickAndReadTextFileAsync(new []{".gltf"});
            
            try
            {
                root currentRoot = JsonUtility.FromJson<root>(json);
                ...
                return currentRoot._marksInfo._marks
                                  .Select(m => new Mark
                                  {
                                      Name = m.name,
                                      Position = new Vector3(m.x, m.y, m.z),
                                      Rotation = Quaternion.Euler(m.rotationX, m.rotationY, m.rotationZ)
                                  })
                                  .ToList();
            }
            catch
        ...
        private static void AddCollision(GameObject currentNodeGameObject)
        {
        ...
        private static void FormMeshesFromGLTF(GameObject currentNodeGameObject, node currentNode, root rootOfGLTFFile)
        {
