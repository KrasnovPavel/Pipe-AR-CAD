...
namespace HoloCAD
{
    public static class LegacyTubeSerializer
    {
        ...
        public static byte[] SerializeScheme(IEnumerable<TubeView> tubes)
        {
            ExpTubesArray array = new ExpTubesArray();
            foreach (TubeView tube in tubes)
            {
                ExpTube expTube = new ExpTube();
                array.tubes.Add(expTube);
                expTube.diameter = tube.tube.Diameter * 1000;
                expTube.width = expTube.diameter * 0.1;
                expTube.fragments.AddRange(tube.PointViews.Select(ConvertPointToExportFormat));
                expTube.fragments.AddRange(tube.SegmentViews.Select(ConvertSegmentToExportFormat));
        ...
        private static ExpFragment ConvertPointToExportFormat(PointView fragment)
        {
            ExpFragment expFragment = new ExpFragment();

            expFragment.type          = "Bended";
            expFragment.bendAngle = fragment.Point.GetBendAngle();
            expFragment.radius    = fragment.Point.BendRadius * 1000;

            expFragment.transform = SerializeTransform(fragment.BendView.transform);
            return expFragment;
        }
        
        private static ExpFragment ConvertSegmentToExportFormat(SegmentView fragment)
        {
            ExpFragment expFragment = new ExpFragment();
            expFragment.type   = "Direct";
            expFragment.length = fragment.segment.TubeLength * 1000;

            expFragment.transform = SerializeTransform(fragment.DirectTubeView.transform);
            return expFragment;
        }
        ...
        /// <returns> Матрица переноса 4х4. </returns>
        private static double[] SerializeTransform(Transform fragment)
        {
            double[] result = new double[16];
            var      scale  = fragment.localScale;
            var      parent = fragment.parent;
            fragment.parent = null;
            fragment.localScale = Vector3.one;
            Matrix4x4 tr     = fragment.localToWorldMatrix;
        ...
        private class ExpTube
        {
            ...
            /// <summary> Матрица расположения трубы. </summary>
